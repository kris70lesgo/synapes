...
type ColorBendsProps = {
  ...
  parallax?: number;
  noise?: number;
  brightness?: number; // multiplies color intensity
  contrast?: number;   // 1.0 = neutral
  saturation?: number; // 1.0 = neutral
  bias?: number;       // additional positional shift added to the original baseline
};
...
void main() {
  ...
  q /= max(uScale, 0.0001);
  q /= 0.5 + 0.2 * dot(q, q);
  q += 0.2 * cos(t) - 7.56 + uBias; // restore original baseline and apply optional bias
  ...
  q += toward * uMouseInfluence * 0.2;
    ...
    // Apply simple filmic adjustments
